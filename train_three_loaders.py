...
from data_three import get_dataloaders
...
def main():
    ...
    # dataloaders
    train_loader1, train_loader2, train_loader3, test_loader = \
        get_dataloaders(args.dataset, args.data_path, args.batch_size, args.num_workers)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss_ce, train_loss_kld = \
            train(train_loader1, train_loader2, train_loader3, model, criterion, criterion_kld, optimizer, epoch)
...
def train(train_loader1, train_loader2, train_loader3, model, criterion, criterion_kld, optimizer, epoch):
    ...
    pbar = tqdm(enumerate(train_loader1), total=len(train_loader1))
    model.train()
    pbar.set_description(f'Epoch [{epoch}/{args.epochs}] training SLL branch')

    totaltargets = torch.tensor([]).long().to(device)
    for i, (images, labels, idx) in pbar:
        images = images.to(device)
        labels = labels.to(device)
        totaltargets = torch.cat((totaltargets, labels.unsqueeze(0)), dim=0)
    ...
    entropy = torch.distributions.Categorical(probs=totalprobs, validate_args=False).entropy()

    newtargets = flatten_targets.clone()
    #Relabeling
    for ind in range(totalprobs.size(0)):
        maxind = torch.argmax(totalprobs[ind])
        if maxind != flatten_targets[ind] and totalprobs[ind][maxind] > 0.5:
            newtargets[ind] = maxind

    newtargets = newtargets.view(-1, args.batch_size)

    model.train()
    pbar = tqdm(enumerate(train_loader3), total=len(train_loader3))
    ...
    for i, (images, labels, idx) in pbar:
        ...
        sub_probs = totalprobs[i * args.batch_size:(i + 1) * args.batch_size]
        weights = entropy[i*args.batch_size:(i+1)*args.batch_size]
        weights = (weights - weights.min()) / (weights.max()-weights.min())
        weights = (1 - weights) * (1 - args.min_weight) + args.min_weight

        weights = weights.unsqueeze(1)
        ldl_target = LD[newtargets[i]] * weights + sub_probs * (1 - weights)
...
if __name__ == '__main__':
    t1, t2, t3, vl = get_dataloaders(dataset='affectnet', data_path=r'E:\datasets\AffectNet\processed')
    main()